...
app.get('/api/trigger-alert', async (req, res) => {
    ...
    const lat = 40.7300 + (Math.random() * 0.05 - 0.025);
    const lng = -73.9850 + (Math.random() * 0.04 - 0.02);
    
    const types = ['Major Traffic Accident', 'Severe Roadworks 🚧', 'Flash Flood Warning 🌊', 'Police Cordon 🚓'];
    const type = types[Math.floor(Math.random() * types.length)];

    let currentHazard = {
        message: `⚠️ GLOBAL ALERT: High delay probability detected due to ${type}. AI is recalculating Matrix routing and adding risk factors.`,
        coord: [lat, lng],
        env: { condition: type, severity: "Critical" }
    };

    packages.forEach(p => p.urgency = 'High'); 

    await triggerAIOptimization();
    res.json(currentHazard);
});
